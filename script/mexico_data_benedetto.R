...
agroproductos1 <- data_agr%>% 
  group_by(hogar) %>%
  summarise(n_agr=sum(count_prod),cant_agr=sum(cantidad),cven_agr=sum(cant_venta),val_agr=sum(valor),ven_agr=sum(val_venta))
rm(data_agr)
data_ani <- data[data$tipo==2 & !is.na(data$valor),]#livestock
agroproductos2 <- data_ani%>% 
  group_by(hogar) %>%
  summarise(n_ani=sum(count_prod),cant_ani=sum(cantidad),cven_ani=sum(cant_venta),val_ani=sum(valor),ven_ani=sum(val_venta))
rm(data_ani)
data_prodani <- data[data$tipo==3 & !is.na(data$valor),]#animal products
agroproductos3 <- data_prodani%>% 
  group_by(hogar) %>%
  summarise(n_prodani=sum(count_prod),cant_prodani=sum(cantidad),cven_prodani=sum(cant_venta),val_prodani=sum(valor),ven_prodani=sum(val_venta))
rm(data_prodani)
data_for <- data[data$tipo==4 & !is.na(data$valor),]#forest products
agroproductos4 <- data_for%>% 
  group_by(hogar) %>%
  summarise(n_for=sum(count_prod),cant_for=sum(cantidad),cven_for=sum(cant_venta),val_for=sum(valor),ven_for=sum(val_venta))
rm(data_for)
data_fis <- data[data$tipo==5 & !is.na(data$valor),]#fishing products
agroproductos5 <- data_fis%>% 
  group_by(hogar) %>%
  summarise(n_fis=sum(count_prod),cant_fis=sum(cantidad),cven_fis=sum(cant_venta),val_fis=sum(valor),ven_fis=sum(val_venta))
rm(data_fis)
data_hun <- data[data$tipo==6 & !is.na(data$valor),]#hunting products
agroproductos6 <- data_hun%>% 
  group_by(hogar) %>%
  summarise(n_hun=sum(count_prod),cant_hun=sum(cantidad),cven_hun=sum(cant_venta),val_hun=sum(valor),ven_hun=sum(val_venta))
rm(data_hun)
...
agroproductos$n_agr <- ifelse(is.na(agroproductos$n_agr),0,agroproductos$n_agr)
agroproductos$cant_agr <- ifelse(is.na(agroproductos$cant_agr),0,agroproductos$cant_agr)
agroproductos$cven_agr <- ifelse(is.na(agroproductos$cven_agr),0,agroproductos$cven_agr)
agroproductos$val_agr <- ifelse(is.na(agroproductos$val_agr),0,agroproductos$val_agr)
agroproductos$ven_agr <- ifelse(is.na(agroproductos$ven_agr),0,agroproductos$ven_agr)

agroproductos <- left_join(agroproductos,agroproductos2,by="hogar")
agroproductos$n_ani <- ifelse(is.na(agroproductos$n_ani),0,agroproductos$n_ani)
agroproductos$cant_ani <- ifelse(is.na(agroproductos$cant_ani),0,agroproductos$cant_ani)
agroproductos$cven_ani <- ifelse(is.na(agroproductos$cven_ani),0,agroproductos$cven_ani)
agroproductos$val_ani <- ifelse(is.na(agroproductos$val_ani),0,agroproductos$val_ani)
agroproductos$ven_ani <- ifelse(is.na(agroproductos$ven_ani),0,agroproductos$ven_ani)

agroproductos <- left_join(agroproductos,agroproductos3,by="hogar")
agroproductos$n_prodani <- ifelse(is.na(agroproductos$n_prodani),0,agroproductos$n_prodani)
agroproductos$cant_prodani <- ifelse(is.na(agroproductos$cant_prodani),0,agroproductos$cant_prodani)
agroproductos$cven_prodani <- ifelse(is.na(agroproductos$cven_prodani),0,agroproductos$cven_prodani)
agroproductos$val_prodani <- ifelse(is.na(agroproductos$val_prodani),0,agroproductos$val_prodani)
agroproductos$ven_prodani <- ifelse(is.na(agroproductos$ven_prodani),0,agroproductos$ven_prodani)

agroproductos <- left_join(agroproductos,agroproductos4,by="hogar")
agroproductos$n_for <- ifelse(is.na(agroproductos$n_for),0,agroproductos$n_for)
agroproductos$cant_for <- ifelse(is.na(agroproductos$cant_for),0,agroproductos$cant_for)
agroproductos$cven_for <- ifelse(is.na(agroproductos$cven_for),0,agroproductos$cven_for)
agroproductos$val_for <- ifelse(is.na(agroproductos$val_for),0,agroproductos$val_for)
agroproductos$ven_for <- ifelse(is.na(agroproductos$ven_for),0,agroproductos$ven_for)

agroproductos <- left_join(agroproductos,agroproductos5,by="hogar")
agroproductos$n_fis <- ifelse(is.na(agroproductos$n_fis),0,agroproductos$n_fis)
agroproductos$cant_fis <- ifelse(is.na(agroproductos$cant_fis),0,agroproductos$cant_fis)
agroproductos$cven_fis <- ifelse(is.na(agroproductos$cven_fis),0,agroproductos$cven_fis)
agroproductos$val_fis <- ifelse(is.na(agroproductos$val_fis),0,agroproductos$val_fis)
agroproductos$ven_fis <- ifelse(is.na(agroproductos$ven_fis),0,agroproductos$ven_fis)

agroproductos <- left_join(agroproductos,agroproductos6,by="hogar")
agroproductos$n_hun <- ifelse(is.na(agroproductos$n_hun),0,agroproductos$n_hun)
agroproductos$cant_hun <- ifelse(is.na(agroproductos$cant_hun),0,agroproductos$cant_hun)
agroproductos$cven_hun <- ifelse(is.na(agroproductos$cven_hun),0,agroproductos$cven_hun)
agroproductos$val_hun <- ifelse(is.na(agroproductos$val_hun),0,agroproductos$val_hun)
agroproductos$ven_hun <- ifelse(is.na(agroproductos$ven_hun),0,agroproductos$ven_hun)

rm(agroproductos1,agroproductos2,agroproductos3,agroproductos4,agroproductos5,agroproductos6)

#total turnover and output
attach(agroproductos)
agroproductos$ven_tot <- ven_agr+ven_ani+ven_prodani+ven_for+ven_fis+ven_hun
agroproductos$val_tot <- val_agr+val_ani+val_prodani+val_for+val_fis+val_hun
...
agroproductos$n_tipo_prod[(val_agr>0 & val_ani+val_prodani>0) & val_agr+val_ani+val_prodani>val_tot*(2/3)] <- 3 #mixed crops-livestock farms
agroproductos$n_tipo_prod[(val_agr>0 | val_ani+val_prodani>0) & val_agr+val_ani+val_prodani<val_tot*(2/3)] <- 4 #mixed farm-primary
agroproductos$n_tipo_prod[( val_agr+val_ani+val_prodani==0) & val_tot>0] <- 5 #primary non-farm
detach(agroproductos)
...
data <- data[data$destino == 1#households' consumption
             | data$destino==3#household's debt repayment
             | data$destino>5,]#household's gifts and products exchanges
...
agroconsumo$val_agr <- ifelse(is.na(agroconsumo$val_agr),0,agroconsumo$val_agr)
...
agroconsumo$val_ani <- ifelse(is.na(agroconsumo$val_ani),0,agroconsumo$val_ani)

agroconsumo <- left_join(agroconsumo,agroconsumo3,by="hogar")
agroconsumo$n_prodani <- ifelse(is.na(agroconsumo$n_prodani),0,agroconsumo$n_prodani)
agroconsumo$val_prodani <- ifelse(is.na(agroconsumo$val_prodani),0,agroconsumo$val_prodani)
...
agroconsumo$val_for <- ifelse(is.na(agroconsumo$val_for),0,agroconsumo$val_for)
...
agroconsumo$val_fis <- ifelse(is.na(agroconsumo$val_fis),0,agroconsumo$val_fis)
...
agroconsumo$val_hun <- ifelse(is.na(agroconsumo$val_hun),0,agroconsumo$val_hun)

rm(agroconsumo1,agroconsumo2,agroconsumo3,agroconsumo4,agroconsumo5,agroconsumo6)

#total self-consumption
attach(agroconsumo)
agroconsumo$n_tot <- n_agr+n_ani+n_prodani+n_for+n_fis+n_hun
agroconsumo$valestim <- val_agr+val_ani+val_prodani+val_for+val_fis+val_hun
detach(agroconsumo)
...
data$apoyo_pago <- (data$apoyo_1+data$apoyo_2+data$apoyo_3+data$apoyo_7)*12#Apoyo con pago
...
data$apoyo_npago <- (data$apoyo_4+data$apoyo_5+data$apoyo_6+data$apoyo_8)*12#Apoyo sin pago
...
data$nvo1 <- ifelse(data$nvo_prog1==2001|data$nvo_prog1==2002,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2001|data$nvo_prog2==2002,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2001|data$nvo_prog3==2002,data$nvo_cant3,0)
data$sembr_vida <- (data$nvo1+data$nvo2+data$nvo3)*12#Sembrando Vida

data$nvo1 <- ifelse(data$nvo_prog1==2003|data$nvo_prog1==2004,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2003|data$nvo_prog2==2004,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2003|data$nvo_prog3==2004,data$nvo_cant3,0)
data$tand_bien <- (data$nvo1+data$nvo2+data$nvo3)*12#Tandas para el Bienestar (Microcréditos para el Bienestar)

data$nvo1 <- ifelse(data$nvo_prog1==2005|data$nvo_prog1==2006,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2005|data$nvo_prog2==2006,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2005|data$nvo_prog3==2006,data$nvo_cant3,0)
data$agromercados <- (data$nvo1+data$nvo2+data$nvo3)*12#Agromercados Sociales y Sustentables

data$nvo1 <- ifelse(data$nvo_prog1==2007|data$nvo_prog1==2008,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2007|data$nvo_prog2==2008,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2007|data$nvo_prog3==2008,data$nvo_cant3,0)
data$precios_gar <- (data$nvo1+data$nvo2+data$nvo3)*12#Precios de Garantía a Productos Alimentarios Básicos

data$nvo1 <- ifelse(data$nvo_prog1==2009|data$nvo_prog1==2010,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2009|data$nvo_prog2==2010,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2009|data$nvo_prog3==2010,data$nvo_cant3,0)
data$credito_gan <- (data$nvo1+data$nvo2+data$nvo3)*12#Crédito Ganadero a la Palabra

data$nvo1 <- ifelse(data$nvo_prog1==2011|data$nvo_prog1==2012,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2011|data$nvo_prog2==2012,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2011|data$nvo_prog3==2012,data$nvo_cant3,0)
data$nacion_fer <- (data$nvo1+data$nvo2+data$nvo3)*12#Nacional de Fertilizantes

data$nvo1 <- ifelse(data$nvo_prog1==2013|data$nvo_prog1==2014,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2013|data$nvo_prog2==2014,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2013|data$nvo_prog3==2014,data$nvo_cant3,0)
data$desarollo_rur <- (data$nvo1+data$nvo2+data$nvo3)*12#Desarrollo Rural

data$nvo1 <- ifelse(data$nvo_prog1==2015|data$nvo_prog1==2016,data$nvo_cant1,0)
data$nvo2 <- ifelse(data$nvo_prog2==2015|data$nvo_prog2==2016,data$nvo_cant2,0)
data$nvo3 <- ifelse(data$nvo_prog3==2015|data$nvo_prog3==2016,data$nvo_cant3,0)
data$otros_prog <- (data$nvo1+data$nvo2+data$nvo3)*12#Otros programas sociales

#direct payments new social programmes
data$nvo_tot_npago <- data$sembr_vida+data$agromercados+data$precios_gar+data$nacion_fer+data$desarollo_rur+data$otros_prog

#farm gross output, net income, direct payments
data$size_val <- (data$ventas_tri+data$auto_tri+data$otros_tri)*4
data$fni_year <- (data$ing_tri-data$ero_tri)*4+data$apoyo_npago+data$pro_agrogan+data$nvo_tot_npago
data$support <- data$apoyo_npago+data$pro_agrogan+data$nvo_tot_npago

#dataset with new agro variables at the household level
agro <- data%>%
  group_by(hogar) %>%
  summarise(n_act=sum(count_member),
            n_fni=sum(fni_year),
...
agro$n_autoconsumo <- ifelse(agro$n_size_val>0,(agro$auto_tri*4)/agro$n_size_val,0)

#share of support on total revenues
agro$n_apoyo <- 0
agro$n_apoyo <- ifelse(agro$n_size_val+agro$n_support>0 & agro$n_size_val>0 ,agro$n_support/(agro$n_size_val+agro$n_support),agro$n_apoyo)
agro$n_apoyo <- ifelse(agro$n_size_val+agro$n_support>0 & agro$n_size_val==0, 1, agro$n_apoyo)
...
#agro
imp <- agro[c("hogar","n_fni","n_size_val","n_size_class","n_tipo_prod","n_autoconsumo","n_autoconsumo2","n_apoyo",
                 "n_apoyo_npago","n_pro_agrogan","n_nvo_tot","n_support",
                 "n_sembr_vida","n_tand_bien","n_agromercados","n_precios_gar","n_credito_gan","n_nacion_fert","n_desarollo_rur","n_otros_prog")]
...
data$n_ing_cor <- data$n_fni+data$n_ingr_noagr+data$n_trabajo+data$n_rentas+data$n_transfer+data$n_estim_alqu+data$n_otros_ing
data$n_tipo_act <- ifelse(data$n_tipo_prod<5,1,0) #households with farming activities

exclude <- names(data) %in% c("folioviv","foliohog","trabajo","rentas","transfer","estim_alqu","otros_ing")
data <- data[!exclude]
rm(exclude)
