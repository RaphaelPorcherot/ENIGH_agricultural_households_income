...
basenames <- c(
  bn_autocons_prod = "ratio_autocons_prod_decile",
  bn_share_support = "share_support",
  bn_support_ing_cor = "ratio_support_ing_cor_decile",
  bn_support_ftr1 = "ratio_support_ftr1_decile",
  bn_support_fni = "ratio_support_fni_decile"
)
...
stopifnot(length(basenames) <= length(list_cols)) # guard

col_scheme <- setNames(
  list_cols[seq_along(basenames)],
  basenames
)
...
basename <- basenames[["bn_autocons_prod"]]
col_below <- col_scheme[[basename]][[1]]
col_above <- col_scheme[[basename]][[2]]
...
basename <- basenames[["bn_support_ing_cor"]]
...
basename <- basenames[["bn_support_ftr1"]]
...
basename <- basenames[["bn_support_fni"]]
...
caption_micro <- paste(
  ...
  "Each household contributes equally regardless of ",den_name," size.",
  "Source: Based on ENIGH data.",
  sep = "\n"
)
